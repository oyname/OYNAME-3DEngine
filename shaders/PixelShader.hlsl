...
float4 main(PS_INPUT input) : SV_Target
{
    ...
    float roughness = saturate(gPbr.y);
    ...
    // AO / ORM (Prioritaet: ORM > separate Occlusion)
    float ao = 1.0f;
    if ((flags & MF_USE_ORM_MAP) != 0u)
    {
        // Legacy: bestehendes Verhalten beibehalten (AO aus orm.g)
        float3 orm = gORM.Sample(gSampler, uv0).rgb;
        ao = lerp(1.0f, orm.g, saturate(occlusionStrength));
    }
    else if ((flags & MF_USE_OCCLUSION_MAP) != 0u)
    {
        float occ = gOcclusion.Sample(gSampler, uv0).r;
        ao = lerp(1.0f, occ, saturate(occlusionStrength));
    }

    // Separate Roughness/Metallic (nur wenn kein ORM gesetzt ist)
    if ((flags & MF_USE_ORM_MAP) == 0u)
    {
        if ((flags & MF_USE_ROUGHNESS_MAP) != 0u)
            roughness = saturate(roughness * gRoughness.Sample(gSampler, uv0).r);

        if ((flags & MF_USE_METALLIC_MAP) != 0u)
            metallic = saturate(metallic * gMetallic.Sample(gSampler, uv0).r);
    }
